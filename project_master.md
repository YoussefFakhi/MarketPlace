...
You are a senior Vue 3 frontend developer. You are building a Single Page Application that integrates with an existing Laravel 12 REST API. Below is the complete, authoritative specification of the backend. You MUST follow it exactly.
...
- Login and Register **BOTH** return a **plaintext token** and the **user object**.
...
- **Validation:** `password` min 4 characters (backend constraint).
- **Response (200):**
```json
{
  "token": "4|xyz789...",
...
```
- **Note:** Register **now returns a token**. You can log the user in immediately.
...
### 4.2 Token Handling
- Login & Register response: Store `token` and `user` (no extra data wrapping).
- No need to chain login after register anymore.
...
| 3 | Password min length 4 | Enforce `max="4"` in input fields. |
...
- `orders.last_reminder_sent_at`: Timestamp (nullable).
